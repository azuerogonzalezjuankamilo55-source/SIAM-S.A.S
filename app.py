...
def configure_logging(app: Flask) -> None:
    ...
    if not app.debug:
        file_handler = logging.FileHandler("siam.log")
        file_handler.setLevel(logging.WARNING)
        file_handler.setFormatter(formatter)
        logger.addHandler(file_handler)

    app.logger.info("SIAM iniciado en modo %s", "produccion" if not app.debug else "desarrollo")
...
def register_error_handlers(app: Flask) -> None:
    logger = logging.getLogger("siam.error")
    ...
    def handle_siam_exception(error: SIAMException) -> Any:
        logger.warning("SIAMException: %s", error.description)
